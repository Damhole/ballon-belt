...
|-------|--------|-------|----|
| v74.45 | (pending) | 2026-05-19 | **SW network-first pro VŠE.** Předchozí cache-first strategie pro assety s `ignoreSearch:true` ignorovala cache-bust query params → starý JS servírován i po push nové verze (PWA i regular browser viděly nový version badge ale starý kód). Fix: SW teď network-first pro úplně všechno, cache slouží POUZE jako offline fallback při network failure. Žádný precache. Bump verze invaliduje starý cache. |
