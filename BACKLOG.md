...
|-------|--------|-------|----|
| v74.38 | (pending) | 2026-05-19 | **PWA standalone — 100lvh místo 100dvh.** iOS PWA standalone (Add to Home Screen) reportuje `100dvh` jinak než real viewport → pruh dole (původně bílý, po v74.37 černý). Fix: `@media (display-mode: standalone)` přepne `html, body` na `100lvh` (largest viewport height — žádný browser chrome v PWA mode). |
