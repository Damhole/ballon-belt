...
| P2 | ✅ | S | Hra | **Zvuky** ✅ done v74.9-v74.52 — kompletní audio palette: pop, bounce, shoot 4-var, step (cannon move), suck (hole), carrier click (xylo/pop), denial, music vrstvy (kick/hihat/clap/pad/belt_tick) s adaptive build-up. Pentatonic pitch quantization. |
| P2 | ✅ | XS | Hra | **Poslední pixel větší collider** ✅ done v74.53 — 2× collider jen pro poslední pixel na scéně |
...
| P2 | 💡 | S | Polish | **Editor preview — floor gap u multi-row levelů** — v editor preview iframe (menší vertikální prostor než plné okno) se floor mesh nedotahuje až k belt area, viditelný gap. Hypotéza: `arenaTopCSS` se může dostat nad `skulinaBotCSS` → arch math invertovaný. Pokus o `MIN_ARCH_HEIGHT` clamp (v74.63 dev) nebyl viditelný v editor preview, příčina nejasná. Možný root: jiný path serveru / cache iframe / něco jiného. Neblokuje hru samotnou, jen editor preview. |
...
| Prio | Stav | Vel. | Téma | Nápad |
|------|------|------|------|-------|
| P1 | ✅ | L | 3D | **RTT cache statických pixelů** ✅ v76.01 — statická scéna (pixely+outline+bloky+frame) do WebGLRenderTarget, per frame jen quad + dynamika (dělo/projektily/shardy). Invalidace: destrukce, wave, intro, theme, resize, tier. Diag `?diag=nortt` pro A/B. Detail: plán bright-roaming-matsumoto.md |
...
| P1 | ✅ | XS | Infra | **Zip exclude list** ✅ v75.05 — prod zip 10 MB, z toho ~5,9 MB balast (`js/.bak/` 5 MB, `levels (1).js`, PNG ikony 790 KB, debug.js, test_balloon.html, BufferGeometryUtils.js, .DS_Store) |
...
| Prio | Stav | Vel. | Téma | Nápad |
|------|------|------|------|-------|
| P1 | ✅ | M | Hra | **Aiming pipeline kanónu** ✅ v75.20 — `gridVersion` counter: (a) plný `pickCannonShot` po každém výstřelu ~27×/s (game.js:1702, reset :8596), (b) revalidace locku ray-marchem každý frame (:8503), (c) `steerAfterBounce` 20 úhlů × 600 kroků v jednom framu (:2128). Největší CPU žrout |
...
|-------|--------|-------|----|
| v76.07 | (pending) | 2026-07-28 | **Bottom canvas zpět na 60 fps** (`_BOTTOM_SKIP_EVERY` 4 → 0) — test, jestli je frame-skip z v74.77/78 po RTT cache a v75/76 úsporách ještě potřeba. Kdyby fps na Mi A1 kleslo, vrátit na 4 (jednořádkovka). |
...
| v75.31 | (pending) | 2026-07-28 | **Stale sweep pop/denial animací** — klíče se mazaly jen při renderu svého slotu; nosič zkonzumovaný během animace (klik hned po reveal) nechal klíč v Mapě navěky → `_hasActiveCarrierAnim()` držel updateCarriers + full render do konce levelu. Druhý zdroj symptomu z Mi A1 (první byl funnel warning v75.28). Sweep na začátku updateCarriers (leak sám drží funkci naživu → sweep se vždy dostane ke slovu). |
| v75.30 | (pending) | 2026-07-28 | **LOW tier → bottom DPR 1.0** — setQualityTier na bottom canvasu dřív nastavoval stejný 1.5× cap pro všechny tiery (degrade nic nešetřil). Tier 2 teď 1.0× = 2,25× méně fill-rate; tier 0/1 beze změny. Na slabých telefonech se aktivuje sám přes auto-degrade. |
...
| v74.78 | (pending) | 2026-05-22 | **Bottom canvas 45fps (skip every 4th frame).** Změna z 30fps (v74.77) na ~45fps — kompromis mezi smoothness a perf. `_BOTTOM_SKIP_EVERY = 4` → render 3 ze 4 frames = 60 × 3/4 = 45fps. Frame pattern: render, render, render, skip, render, render, render, skip, ... Time gaps mírně mixed (16.7ms + 33ms) ale visuálně méně stuttery než 30fps. Save ~0.3-0.7ms/frame vs full 60. |
| v74.77 | (pending) | 2026-05-22 | **Bottom canvas 30fps (half rate).** Render+updates bottom canvasu jen každý druhý frame přes `window._bbBottomFrame & 1` check. Wrappnuto v `_BOTTOM_HALF_RATE = true` flag pro snadný revert. Fyzika v game.js (updatePending) běží dál 60fps s 4 substeps — jen vizuální mesh update půlrychlostí. Důsledky: pending balls v letu = 1 frame visual lag (~33ms stutter), belt scroll ~2px jump/frame, mystery texture scroll choppy. Saving: ~0.5-1.5ms/frame na bottom render + GPU bandwidth halved + updatePending/updateBelt/updateCarriers calls halved. Top canvas (image area) zůstává 60fps. |
| v74.76 | (pending) | 2026-05-22 | **Wall cache + pixel count tick disabled.** (1) `updateWalls` hash cache z `bgColor + walls pozice` — walls jsou statické per level, na carrier klik se nemění. Předtím per drawCarriers (= per klik) dispose + rebuild ALL wall ExtrudeGeometries. Cache kicks rebuild jen na theme change nebo level start. Hlavní zdroj click-drop. (2) Per-frame pixel counter (`_remainingPxCache` refresh, 1116 iterations × 60fps = 67k ops/s) wrappnutý do `_ENABLE_PIXEL_COUNT_TICK = false` flagu. Kód zachován pro budoucí use. Důsledek: end-of-level slowdown ramp neaktivní (game stays at full 2× auto-speed do konce). Manual boost + carriers-cleared ramp fungují normálně. |
| v74.75 | (pending) | 2026-05-22 | **Perf: eliminace 1-5s freeze cyklu.** Hledání zdroje "drops 70-80% / 1s + recovery 3-5s" patternu. Dva viníci nalezeni: (1) `setQualityTier` traverse přes scénu + `needsUpdate=true` na všech materiálech při ANY tier change → next frame synchronous shader recompile per material (10+ materials × 100-500ms = 1-5s freeze). Auto-tier MED↔LOW cyklus to spouštěl periodicky → drop → degrade → recompile → bigger drop → recovery → upgrade → recompile cycle. Fix: condition `(shadowsChanged \|\| prev !== t)` → `shadowsChanged` jen. Tier number sám nezmění žádný shader uniform. (2) `updateBlockOutlines` per-frame dispose + create ExtrudeGeometry per blok (drawBlocks volá per pixel destroy = 10-20×/s). Fix: hash cache key `x,y,w,h,hp` — skip rebuild pokud unchanged. Rebuilds jen při HP change nebo block destroy. |
| v74.74 | (pending) | 2026-05-22 | **Belt loading citlivost + nudge off + top canvas zpět 2×.** (1) `findBeltLoadSlot` nový param `preferEmpty=true` v load callu → najde nejbližší PRÁZDNÝ slot v toleranci 1.5× spacing. Předtím vrátil closest regardless of state → ball čekala když byl closest full i když byl vedle prázdný. Gate caller drží `preferEmpty=false` (pure closest). Efekt: balls chytají belt sloty ochotněji, méně push-back pendings. (2) `_NUDGE_ENABLED = false` flag → `nudgeStuckNearOpening` early return. Auto-nudge balls ve trychtýři vypnut — po slot fix by neměl být potřeba. Easy re-enable přes flag pokud reálné stuck cases. (3) Top canvas pixel ratio 1.75× → 2× (zpět plné retina) — B-pool z v74.73 dal headroom z GC eliminace. Bottom canvas drží 1.5×. |
| v74.73 | (pending) | 2026-05-22 | **Perf B: object pools pro 5 particle systémů — eliminace GC pressure.** Free-list pattern napříč: (1) `state.shards` 3D shards render3d.js [4 spawn sites], (2) `state.ghosts` CA ghosts [2 sites], (3) `state.dust` dust motes [1 site], (4) `shards` 2D shards game.js [2 sites], (5) `smokePuffs` game.js [1 site]. Acquire = pop ze pool free-listu (žádný `new` při steady state); release = swap-pop z active arraye + push do free-listu (žádný `splice` shift, žádný garbage). Persistentní `THREE.Color` v každém pool slotu → `.copy()` nebo `.setRGB()` per spawn, eliminuje `new THREE.Color()` a `.clone()` allocs. Reset v `startLevel` releases zpět do poolu before clear (zachová pool capacity). Co nepoolováno (acceptable): `particles` (long-lived gameplay), `confetti` (one-shot win), `_carrierRipples`/`_rippleDots`/`_holeAnims` (Three.js mesh-based, sparse). Expected: GC pressure -80%+ během active gameplay, Minor GC frequency 3-8s → 30-60s, stuttery 5-10× méně časté. Average FPS jen mírně lepší, ale **stabilita ("vrací se zpět" symptom) razantně lepší**. |
| v74.72 | (pending) | 2026-05-22 | **Pixel ratio asymetrický split + boost tip skip v auto-speedu.** (1) Top canvas (image area, render3d.js) pixel ratio cap 1.5× → 1.75× — pixely + bloky = hlavní pozornost hráče, vyšší detail. Bottom canvas (carriers/belt/funnel) drží 1.5× (UI elementy, méně critical). Asymetrický split: top má ~36% víc pixelů než bottom, ale stále 23% méně než původní 2× retina. (2) Boost tutorial tip — early return v `_maybeShowBoostTip` pokud `_carriersClearedAt !== null` (auto-speedup běží, user nemá co zkoušet). Plus re-check uvnitř setTimeout (600ms může proběhnout vyklízení). Plus dismiss visible tip v `_updateUserHold` per-frame tick když auto-speedup naskočí během display. Tři vrstvy ochrany. |
| v74.71 | (pending) | 2026-05-22 | **Perf: pixel ratio cap 1.5× pro VŠECHNY tiery.** Předtím HIGH/MED = `min(dpr, 2)` (na 3x retina = 4× pixel count = drahé fragment shader work). Po: všechny tiery = `min(dpr, 1.5)` (na 3x retina = 2.25× pixel count). 4 místa změna (render3d init + setQualityTier, render3d_bottom init + setQualityTier). Odhad save: 3x devices (iPhone Pro, Galaxy S) ~44% méně fragmentů; 2x devices (běžné mobily) ~44% méně; 1x devices žádná změna. CanvasTexture rendery (HP text, funnel warning) ponechány — to není per-frame cost. Vizuálně: jemnější softer detail na top-tier devices, ale toon shading + bevel + outlines stále drží 3D dojem. |
| v74.70 | (pending) | 2026-05-22 | **Default tier = MED + cap upgrade.** `_perfTier=0 → 1` (default MED hned od startu hry). `_ensureR3D` / `_ensureR3DBottom` po init aplikují `setQualityTier(_perfTier)` na oba renderery → shadows off od první frame. Auto-degrade na LOW pořád funguje (FPS<29 přes 10s). Auto-upgrade cap: LOW→MED OK, **MED→HIGH zablokováno** (`_perfTier>1` místo `>0` v step-up). Manual override `setPerfTier(0)` funguje (override cap). Plus revert v74.70-hack na pixelMesh: `castShadow=true` zpět (řízeno per-tier přes setQualityTier), `receiveShadow=false` zachováno (v74.69). Reálný efekt na MED: shadowMap.enabled=false → žádné shadows i přes castShadow flags na meshech. Tím se hra startuje rovnou s MED výkonem ale visuálně připravená nazpět na HIGH když manual override. |
| v74.69 | (pending) | 2026-05-22 | **Perf: pixelMesh.receiveShadow vypnut.** Pixely přestanou přijímat stíny od bloků/projektilů (PCF lookup eliminován z main pass — ~1M lookups/frame ušetřeno = ~112k fragmentů × 9 PCF samples). Pixely pořád VRHAJÍ stíny na ground/frame. Visual loss: bloky nevrhají tmavý stín dolů na pixely, projektily v letu neházejí stín na image area, pixely nemají self-shadow. Perf save odhad: 0.5–1.5 ms/frame = ~30–40 % celkového shadow costu. |
| v74.68 | (pending) | 2026-05-22 | **Hold-to-boost tutorial tip — full design.** Nový tutorial tip co se zobrazí 1× per device (localStorage `bb-tip-boost-shown`) po prvním funnel warning ("MAX 4 CARRIERS AT ONCE!") fade-out. Komponenty: (1) Bubble — `#fffdf5` BG, 2px dark mauve outline (`#1f1428`), border-radius 14px, stacked drop shadow ("sticker" look), entrance pop spring (cubic-bezier 0.34, 1.56, 0.64, 1) bez continuous pulse. (2) Hand 👆 emoji — v pravé boost zóně (centrované right:35, top -90px nad tip), animace "tap & HOLD" 2.4s cyklus: 0% retracted dole → 12% reach up + press → 12-72% HOLD nahoře (60% cyklu = 1.44s viditelné držení) → 85% retract zpět dolů. (3) Pulsující kruh `::before` pod rukou — bílý outline + 50% opacity white fill, S-curve easing (cubic-bezier 0.65, 0, 0.35, 1), 10% gentle appear (scale 0.08) → 72% peak (scale 1.15) → 90% collapse, jen 4 keyframy bez fáze. Pozice top: calc(60% - 25px) (posunut 25px nad center). Tip přesunut z `#carriers-wrap` do `#bottom-deck` aby seděl v stejné oblasti jako 3D funnel warning text. Dev API `window._bbResetBoostTip()` + ⚙ pill "💡 Reset tip" v dev overlay — vymaže localStorage flag, tip se znovu objeví AŽ při dalším funnel warning (imituje first-time-player flow). Auto-dismiss po 6s OR když user fakticky stiskne boost zónu. |
| v74.67 | (pending) | 2026-05-22 | **Blok text-selection + long-press context menu během gameplay.** Hold gestures (boost zone) trigerovaly browser text selection nebo iOS "Look up / Share / Copy" popup → rušilo gameplay. Fix: CSS `user-select: none` + `-webkit-touch-callout: none` na `#game *` + `#boost-vignette`. Plus JS `contextmenu` preventDefault na boost zones (iOS Safari context menu CSS alone nezablokuje). Žádný side effect na carriers/UI elements — všechny game ovládací prvky stejně používají click/touchstart events. |
| v74.66 | (pending) | 2026-05-22 | **Vignette stronger tint + boost zones responsive height.** Vignette: alpha 0.45 → 0.75, transparent center gradient 30% → 15% (víc tinted area), min opacity pulse 0.35 → 0.45 (vidět i v minimu). Boost zones: `height: 170px` fix → CSS var `--boost-zone-h` (set JS-em). Měření přes `_updateBoostZoneHeight()` (`gridRect.top - deckRect.top - 4`), voláno z `_recomputeCarrierLayout` + na konci `startLevel`. Default fallback 110px. Tím zones končí těsně nad horní řadou carriers nezávisle na počtu řad / shift / responsive velikosti. |
| v74.65 | (pending) | 2026-05-20 | **Boost BG pulse refactor — vignette overlay místo filter na body.** v74.64 použil `filter: brightness saturate` na body — drahé na mobilu (full document recomposite per frame přes WebGL canvas content). Nahrazeno: dedikovaný `<div id="boost-vignette">` fixed overlay, radial gradient (transparent center → warm tint okraje), animovaná **jen opacity** (GPU-optim, no repaint). Z-index 0 → pod canvasy a game contentem, jen nad body BG → pulsuje viditelně okolo hry, ne přes herní content. Vignette opacity 0.35→1.0, 0.40s alternate, will-change:opacity hint. Plus tint vibrancy bumpnut (rgba 0.35 → 0.45). |
| v74.64 | (pending) | 2026-05-20 | **Hold-to-boost 2× s BG pulse + boost zones v rozích.** Dvě průhledné touch zóny (70×170 px) v horních rozích #bottom-deck (vedle funnel arch, nad carriers) — levá + pravá pro praváky/leváky. Hold (touch/mouse down) → game speedMul fade 1.0× → 2.0× za 0.5s. Release/leave → fade zpět. `body.user-boost-active` triggeruje subtle `filter: brightness 1.0→1.06` pulse (0.55s alternate) jako vizuální cue. Integrace v `_speedMul()`: `Math.max(carriers_auto_mul, user_hold_mul)`. Když vyklízení carriers nakopne auto-2× ramp, user hold se automaticky ruší (priorita: auto). Reset stavu v `startLevel`. |
| v74.63 | (pending) | 2026-05-20 | **Per-level intro animation lock + editor preview floor gap (known issue).** Input lock pro startLevel přepracovaný z fixní hodnoty na per-level mapping (`_LEVEL_INTRO_DURATIONS`). Preset levely s build-up animací obrazku dostávají lock matching duration animace: smiley 2.0s, moon 2.2s, starwars 2.4s, frog 3.5s, mondrian 2.5s. Custom levely a vše ostatní jen 700ms (cascade pop coverage). Root cause: během intro anim (`step()` postupně plní `grid[y][x]=color`) by player click → projektil destroy → `grid[y][x]=-1`, ale další animation tick by přepsal zpět → unwinnable. Lock to vyřeší preventivně. BACKLOG: zaznamenán P2 polish "Editor preview floor gap" pro multi-row levely (objevuje se jen v editor preview iframe, hra samotná OK). |
| v74.62 | (pending) | 2026-05-20 | **Flow & pacing balík + editor merge colors + landing page.** Game: (1) Belt +15% (50 → 57.5 units/s). (2) Cannon fire +10% (40 → 36 ms interval). (3) **Speed-up ramp** — když `cntCarriers()===0`, hra zrychlí lineárně 1×→2× za 3s; aplikuje se na belt, cannon fire, projektily, pending balls, cannon move. Animace + hudba real-time. (4) **Slowdown** — na posledních 40→20 pixelech ramp zpět na 1.0× (klidný finish). (5) **Input lock 4s** na startu levelu — zakaz klikání dokud nedoběhne cascade pop + mystery reveal. Editor: **🎨 Photo color minimizer** v pixel toolbaru — checkbox přepne do preview módu, slider (5–120 RGB práh) live updatuje canvas s greedy clustering (anchory = nejčastější barvy), Použít / Zrušit. Plus: **prezentace.html** — landing page draft. | 
| v74.61 | (pending) | 2026-05-20 | **Shoot vol -58% (0.12 → 0.05) — decentnější dělo.** User reportoval pocit "záměny zvuků" — že dělo má pop pixelu a pixely nic. Diagnóza: žádná záměna (soubory + trigger sites OK), ale shoot byl 60% hlasitější než pop a oba jsou "bubble plop" → mozek vnímal dominantní shoot jako "pop pixelu". Místo bumpu popu jsme snížili shoot pod úroveň popu (0.05 < 0.075) → cannon je teď v pozadí, pixel pop dominuje. |
| v74.60 | (pending) | 2026-05-19 | **Mystery reveal — proper layer order + Next button 3D theme fix.** (1) Reveal mesh `depthTest:true → false` (v74.59 přístup "skip balls" byl wrong — user chtěl balls pod coverem, ne pop-in po anim). Teď balls renderují normálně, cover (renderOrder 144) je vždy překryje od t=0, jak wipe roste (uRevealT 0→1) balls se odhalují postupně. Pending balls (renderOrder 150) renderují PO reveal, takže přes něj létají normálně. (2) `body.renderer-3d #next-btn` selector přidaný k `#restart-btn` override (dark bg #3a2330) — v74.59 jsem zapomněl na theme override pro Next, takže button zůstával transparent + bílý text na světlém overlay v 3D módu = neviditelný. |
| v74.59 | (pending) | 2026-05-19 | **Mystery reveal fix + Next button neutral.** (1) Balls v mystery slotu prosvítaly přes reveal wipe animaci (0.45s) — reveal mesh má `depthTest:true` kvůli pending balls (renderOrder 144 vs pending 150), takže carrier balls (renderOrder ~110) failed depth test když reveal měl z=0 a balls z=9.75. Fix v `updateCarriers`: během `mysteryRevealMeshes.has(carrierKey)` skip ball loop pro daný slot — slot + outline render normálně, balls se objeví po dohrání reveal anim. (2) Next button přebarvený do neutrální (stejný styl jako Restart) — modrý accent byl nečitelný na světlém pozadí overlay. |
| v74.58 | (pending) | 2026-05-19 | **End-screen "Další →" button.** Overlay (overlay-box) má teď druhé tlačítko vedle "Hrát znovu" — klik skryje overlay a zavolá stepLevel(1) → posune currentLevel na další v pořadí LEVELS a startne. Cyklí dokola (modulo length). Modré tlačítko (accent blue) odlišené od restart. Pro standalone share-link testy: tester po dohrání svého levelu může pokračovat na další pomocí Next. |
| v74.57 | (pending) | 2026-05-19 | **Editor: in-game filter + level card UI + share link + HP -30%.** Sekce Informace: nový checkbox "🎮 In game" (default ON, odškrtnuto = level zůstane jen v editoru, do hry nepůjde — `writeGameLevelsJs` filtruje `inGame !== false`). Karta levelu přepracovaná: mini thumbnail (56×42, kreslí pixely pro custom / placeholder pro presety), 2-řádkový layout (title nahoře, meta badges #X · 🎮/📝 · diff · status dole), 🔗 share button kopíruje GH Pages link `index_local.html?level=KEY` do clipboardu. HP text v 3D blocích: fontPx 24 → 17 (-30%). |
...
| v74.52 | `df1ea0b` | 2026-05-19 | **🎉 Milestone: Audio + visual feedback session.** Uzavírá rozsáhlou session (v74.9-v74.52) — kompletní audio palette (pop, bounce, shoot 4-var, step, suck, carrier click, denial, kick/hihat/clap/pad/belt_tick), pentatonic pitch quantization napříč SFX, adaptive music s 5 vrstvy build-up, carrier click ripple (shader full→ring→dots), funnel warning 3D theme-aware mesh s Bangers fontem, PWA service worker network-first + auto-reload, iOS PWA viewport fix (100lvh), PNG ikony Plop! brand. Z Beta sprintu vyřízeno jen **Zvuky** — ostatní položky (Blocks fix P0, Editor, FPS drain, Photo minimizer, Theme k levelu, Ball-to-hole check, Last pixel collider, Belt speed, Flow speedup, Ball highlights, Smoke ztlumení, Direct level link) jsou stále otevřené. |
| v74.51 | `b516034` | 2026-05-19 | **Funnel warning — 3D mesh + theme-aware tint + Bangers font.** Místo červeného DOM boxu teď Three.js mesh v 3D scéně. CanvasTexture s textem "MAX 4 CARRIERS AT ONCE!" (Bangers font, Google Fonts), ShaderMaterial s reveal/erase mask přes UV.x (smooth cubic easing). Pevná pozice v world coords (`_worldY(FUN.narrowY + 130)`) = stabilní napříč resize. Text color theme-aware: 90% bílá + 10% `--carriers-3d-bg` současného theme. Redraw na `bb:theme-changed` event. Logic: writing → visible → 2s grace po vyprázdnění trychtýře → erasing → idle. Re-klik během writing/visible = no-op, prodlouží zobrazení implicitně. DOM #funnel-warning skryto (`display:none`). |
| v74.50 | `78c4c5b` | 2026-05-19 | **Ripple animace rychlejší.** Ring duration 380→240ms, dots 450-700→280-460ms. |
| v74.49 | `a9f6ba3` | 2026-05-19 | **Carrier click ripple + fading dots.** Po zmáčknutí nosiče vzniká plný kruh → ring který se zužuje → zmizí ztenčením → na jeho místě se rozprsknou 6-8 teček v barvě nosiče, drift outward 3-7px, fade 450-700ms (t² accelerating). Custom ShaderMaterial pro ring (CircleGeometry + shader: innerR roste 0→0.5, outerR=0.5). sRGB→linear color fix pro správnou barvu. depthTest:false + renderOrder 250 = vždy nad scénou. |
| v74.48 | `8ed3538` | 2026-05-19 | **Carrier click — Pop 29 + fixní melodie + balloon test page.** Pop Sound 29 trim 0.115-0.30s (preskočeno 119ms intro ticho — fix delayed playback). Carrier click teď používá fixní 16-step melodii (`_CARRIER_MELODY = [0,2,3,5,4,3,2,4,5,3,2,0,2,4,3,1]`) v lower-mid pentatonic range (A2-C4) — rozpoznatelná opakující se fráze, žádné vysoké tóny. Plus standalone `test_balloon.html` page pro experimenty s translucent carrier materiály (balloon-bag look — experiment, do hry neimplementováno). |
| v74.47 | `92322a5` | 2026-05-19 | **Revert v74.46 — pixel beat stretch pryč.** Shader injection do pixel materiálu odebrán, uniform updates pryč, `_bbKickBeatTime`/`_bbMusicOn` triggery pryč. Pixely zachovávají jen existující animace: wave-bounce při destrukci + hit-bounce stretch při wrong color hit. Beat-driven pulse byl moc subtilní / nepostřehnutelný. |
| v74.46 | `dea4862` | 2026-05-19 | **Beat-driven pixel stretch — GPU shader.** Pixely se jemně Z-stretch při hudbě, vlna sweep přes mřížku (per-instance offset z world pos). 750ms cycle (quarter note pulse), 8% amplitude, amplituda 0 při music off. Implementace: shader injection do MeshLambertMaterial + uniform update v render() loop. CPU cost prakticky nulový (1 uniform / frame). Plus hole sekvence posunuta níž: `[0.50, 0.595, 0.667, 0.749, 0.84, 0.94]` (A min pentatonic nízká oktáva, těsnější intervaly). |
| v74.45 | `5ec4471` | 2026-05-19 | **SW network-first pro VŠE.** Předchozí cache-first strategie pro assety s `ignoreSearch:true` ignorovala cache-bust query params → starý JS servírován i po push nové verze (PWA i regular browser viděly nový version badge ale starý kód). Fix: SW teď network-first pro úplně všechno, cache slouží POUZE jako offline fallback při network failure. Žádný precache. Bump verze invaliduje starý cache. |
| v74.44 | `1a25757` | 2026-05-19 | **Audio polish — suck pitch melodie, carrier click, music decay.** Suck sound: sekvenční pentatonika `_SUCK_SEQUENCE` (75% sequential, 25% random), reset po 8s, 20% chance harmonic overlap, volume final 0.147. Hole zkrácen na 6 tónů (`_HOLE_SEQUENCE` bez 1.6 a 1.9 — bez pisku). Carrier click (`_playBeltLand`): Pop Button trim 150ms, vol 0.10, sdílí pentatoniku se suck. Plus: music adaptive decay — po 3s idle se layers postupně oholí zpět na kick (belt→pad→clap→hihat), kick zůstává. |
| v74.43 | `52975c0` | 2026-05-19 | **Rhythm fire refactor — snap jen na idle-to-active transition.** Cannon fires normálně rychle. Pouze pokud byl idle >400ms a chce zase pálit → čeká na další music tick (snap). Continuous fire bez rhythm gate. Enhanced muzzle flash (větší/světlejší) na kick beat = nezávislé na rhythm toggle, hraje vždy pokud music on a fire padne na downbeat = "rytmus jen doplňuje, nestrhává". |
| v74.42 | `139823b` | 2026-05-19 | **Fix v74.41 bug (break out of nothing) + air pressure suck zvuk.** v74.41 měl `break` ve fire bloku který nebyl uvnitř loopu → JS chyba → hra nestartovala. Fix: `let _canFire` + `if(_canFire){...}` wrap. Plus: `suck_melody.wav` nahrazen air pressure suction (trim 0.15-0.55s = 400ms, -10dB, fade in/out). `_playSuckMelody` re-enabled (vol 0.30), hraje na všech 4 triggerHoleSuck sites = jemný hiss když balónek mizí do díry. |
| v74.41 | `d32b15e` | 2026-05-19 | **Rhythm fire — soft snap kanonu na music tick + enhanced flash na downbeatech.** Opt-in toggle "🎯 Rhythm fire" v settings. Cannon fire jen ve 100ms okně po každém scheduler ticku (8th note grid 375ms) — fires snap k beatu bez velké pauzy. Když fire padne na _KICK_PATTERN pozici (downbeat) → enhanced muzzle flash: větší scale (0.6→2.4 vs 0.5→1.6), delší trvání (180ms vs 130ms), 60% mix s bílou (vs 35%). Vyžaduje music ON. |
| v74.40 | `207f4dc` | 2026-05-19 | **PWA auto-reload on SW update.** Bug: po deploy nové verze PWA ukazovala nový HTML version badge ale starý cached JS (no music toggle). Důvod: SW cache-first → stará game.js servírována dokud cache neinvaliduje. Fix: `controllerchange` listener v index_local.html → když nový SW převezme kontrolu, page se sama reloadne pro fresh assety. Žádné víc ručního reinstall PWA. |
| v74.39 | `0450178` | 2026-05-19 | **Music master toggle — default OFF.** Adaptive music (kick/hihat/clap/pad/belt_tick) je teď opt-in. `_musicEnabled` flag perzistovaný v localStorage `bb-music`. Toggle v dev overlay (⚙) — "🎵 Hudba" pill vedle Safe zone. Default SFX (pop/bounce/shoot/step) běží na full vol jak před adaptive music. Plus clap volume -30% (0.90 → 0.63). |
| v74.38 | `cf3ca65` | 2026-05-19 | **PWA standalone — 100lvh místo 100dvh.** iOS PWA standalone (Add to Home Screen) reportuje `100dvh` jinak než real viewport → pruh dole (původně bílý, po v74.37 černý). Fix: `@media (display-mode: standalone)` přepne `html, body` na `100lvh` (largest viewport height — žádný browser chrome v PWA mode). |
| v74.37 | `e2559ca` | 2026-05-19 | **Fix iOS PWA white strip viewport bug.** Při PWA standalone launch iOS Safari hlásil chybnou viewport výšku → 50-100px bílý pruh dole do první user interakce. Fix: `html` element dostal `background: #111` + `min-height` s `-webkit-fill-available` fallback (iOS specific). Tím i pokud body height vypočítá iOS špatně, html pozadí pokryje viewport. |
| v74.36 | `01056d9` | 2026-05-19 | **PWA service worker — offline-first caching.** `gamee/sw.js` cachuje critical bootstrap (HTML/JS/CSS/Three.js libs) na install, opportunisticky assety (audio/GLB/images) na fetch. Network-first pro HTML, cache-first pro assety, `ignoreSearch` ignoruje `?t=`/`?v=` cache-bust. Cache versioning vázané na verzi → bump verze invaliduje starý cache. Registrace jen v `index_local.html` (PWA entry), Gamee zip exclude rozšířen o `sw.js`. CLAUDE.md bump checklist rozšířen o sw.js. Vyřeší PWA "missing assets" issue + offline launch. |
| v74.35 | `2e89b6c` | 2026-05-19 | **Belt tick (5. vrstva) + SFX fade s music completeness.** Belt tick (soft wood-block 320Hz, 70ms) jako 5. vrstva — pattern `. T . T . T . T` (off-beaty), unlock po padu, ramp ~8s. Plus `_getSfxFade()` ztiší pop/bounce/shoot/step jak music narůstá (1.0 → 0.60 při full music) — gameplay SFX se "vsadí" do kompozice, music dostane prostor promluvit. |
| v74.34 | `1b16349` | 2026-05-19 | **Adaptive music — sekvenční layer build-up (4 vrstvy).** Kick (synth 55Hz sine + pink noise transient) + hihat (filtered noise) + double-clap + ambient pad (Am7, 7s, random pitch z pentatoniky, 35% chance overlapping druhý pad). Pattern 80 BPM 8-step. Build: kick (25s) → hihat (19s) → clap (14s) → pad (11s) ~70s total. Spouští se 1. výstřelem, zůstává během pauz, reset na novém levelu. |
| v74.33 | `8a4d78b` | 2026-05-19 | **Pop fatigue volume drop + throttle 200ms.** V rychlé sérii popů gain klesá -25% per consecutive (1.0 → 0.75 → 0.56 → ... floor 0.05). Reset na full po >1s pauze. Throttle 60→200ms aby se popy nepřekrývaly a každý dB drop byl jasně slyšet. |
| v74.32 | `44bd1d1` | 2026-05-19 | **Pop: měkčí výšky.** `_wireAndPlay` přijímá volitelný per-zvuk lowpass cutoff. Pop dostal vlastní lowpass na 1000Hz (vs globální 2500Hz na bus) → tlumené výšky, jemnější transient. Ostatní zvuky beze změny. |
| v74.31 | `25db603` | 2026-05-19 | **Audio: foley step slide pro cannon move + relax variant pipeline.** Pop/bounce/shoot pentatonic minor, lowpass 2500Hz, stereo pan ±0.4, soft attack envelope 8ms (= rain/ocean feel). Cannon move: foley step slide (high heels concrete) bez tvrdého heel clicku — trim start 0.140s, fade-in 50ms, hraje jen na pohyby >100px, sequencer (žádné překryvy). Pizz, clash, suck melody disabled (čeká na lepší zvuky). |
...
| v74.21 | (pending) | 2026-05-18 | **Fix pop hlasitost — burst call site.** Burst (`_playPop(0.5)`) ignoroval default → ztišeno na 0.18. Single pixel zůstává na 0.22 default. |
| v74.20 | (pending) | 2026-05-18 | **Pop zvuk: hlasitost znovu o polovinu níž.** 0.45 → 0.22. |
| v74.19 | (pending) | 2026-05-18 | **Pop zvuk: jemnější + 50% skip.** vol 0.7 → 0.45, throttle 40 → 60ms, `Math.random() < 0.5` skip — při rychlé sérii zásahů zvuk neotravuje. |
