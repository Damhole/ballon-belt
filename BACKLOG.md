...
| v70 batch 1: 2.5D vizuální upgrade — Fáze 1 (Three.js scaffolding + 3D pixely) — nový modul `gamee/js/render3d.js` (ESM, Three.js r170 přes importmap), `OrthographicCamera` 1:1 zarovnaná s grid (Y-flipped frustum), `InstancedMesh` `BoxGeometry` pro pixely (max 972 instances) s per-instance color attribute, procedurální bevel CanvasTexture (gradient highlight nahoře, AO dole, diagonal light, výrazné top/left edge bevels, glossy specular blob) na top face, AmbientLight 0.85 + DirectionalLight key 0.6 (upper-left-front) + fill 0.25 (opposite). RENDERER_MODE flag (`?renderer=3d` v URL, default 2D safe fallback). Nové canvas `#three-canvas` (z-index:1, alpha) překryje `pixel-canvas`, který v 3D módu drží jen 2D bloky (Fáze 2 je posune). Per-frame render() volá beltLoop. Tilt scene group 4° (jemný náznak hloubky). AI tester + editor + postMessage protokol nedotčené. 60 FPS na desktopu, žádné runtime shadows (mobile-friendly). | `d2f76d1` | 2026-05-07 |
