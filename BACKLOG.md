...
|-------|--------|-------|----|
| v73.340 | (pending) | 2026-05-18 | **BG linear gradient (radial → linear).** Main body background změněn z `radial-gradient(ellipse at top, top → bottom)` na `linear-gradient(to bottom, top → bottom)` v `body.renderer-3d` CSS. Světlejší barva nahoře, tmavší dole, vertikální plynulý přechod. Vignette layer (radial dark edges) zachován. |
