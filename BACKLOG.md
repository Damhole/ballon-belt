...
|-------|--------|-------|----|
| v74.1 | (pending) | 2026-05-18 | **🐛 PWA fix: empty UI / Three.js loading failure.** V PWA standalone modu uživatel viděl jen pozadí + prázdný image-area (žádné belt/carriers/pending). Root cause: `body.renderer-3d` CSS skrývá 2D fallback elementy (belt-svg, pending-canvas, .cbox > div) protože je nahrazuje Three.js. Když Three.js moduly selžou load (typicky importmap unsupported v iOS Safari < 16.4 nebo některých PWA contextech), 3D scéna se nevyrenderuje a 2D zůstává schované → nic vidět. **Fix 1**: přidán `es-module-shims` polyfill před importmap → fixuje importmap support na starších browserech. **Fix 2**: defensive fallback — pokud `window.render3d` není ready po 2s, přidá `body.three-fallback` class → CSS re-revealne 2D elementy + game loop začne kreslit 2D belt/pending. |
