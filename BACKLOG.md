...
|-------|--------|-------|----|
| v74.80 | (pending) | 2026-05-31 | **Unified canvas — sloučení 2 WebGL kontextů do 1 (experiment, větev distracted-dewdney).** Nový orchestrátor `render3d_unified.js`: jeden sdílený `WebGLRenderer` + `Scene` + `OrthographicCamera` (CSS-pixel prostor `#game`, y-up). Top (render3d.js) i bottom (render3d_bottom.js) se reparentují jako "placement group" subtree → jeden render pass, jeden sdílený světelný rig (hemi+sun+fill, 5→3 světla), místo 2 kontextů + 2 compositor vrstev + 2 passů. Tilt je transformace grupy (ne kamery) → vnitřní matematika obou modulů beze změny. Bottom canvas zůstal jako transparentní "ghost" = měřící reference pro carrier alignment (žádný kontext). Init oba moduly přijímají `shared={renderer,scene,camera,mount}`, render()/resize()/setQualityTier guardují sdílené zdroje. **Měření: na CPU throttle 20× ŽÁDNÝ FPS rozdíl vs dual-canvas → hra je CPU-bound, dvojí kontext nebyl bottleneck.** Nasazeno jako `gamee-test/` na GH Pages pro mobilní test (GPU/teplo, co throttle neměří). Další: profilovat CPU žrouty; zvážit keep vs revert dle mobilního testu. |
