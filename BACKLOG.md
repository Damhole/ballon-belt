...
|-------|--------|-------|----|
| v74.36 | (pending) | 2026-05-19 | **PWA service worker — offline-first caching.** `gamee/sw.js` cachuje critical bootstrap (HTML/JS/CSS/Three.js libs) na install, opportunisticky assety (audio/GLB/images) na fetch. Network-first pro HTML, cache-first pro assety, `ignoreSearch` ignoruje `?t=`/`?v=` cache-bust. Cache versioning vázané na verzi → bump verze invaliduje starý cache. Registrace jen v `index_local.html` (PWA entry), Gamee zip exclude rozšířen o `sw.js`. CLAUDE.md bump checklist rozšířen o sw.js. Vyřeší PWA "missing assets" issue + offline launch. |
