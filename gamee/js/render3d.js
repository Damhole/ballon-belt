...
// v74.77: version stamp pro watchdog — game.js compare proti tomuto
if (typeof window !== 'undefined') window.BB_VERSION_R3D = 'v74.77';
...
// v74.77: Object pools pro particles — eliminuje GC pressure.
...
function init(canvas, opts) {
  ...
  state.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2));  // v74.77: top canvas zpět 2× (B-pool dal headroom), bottom drží 1.5×
  ...
  // v74.77: receiveShadow vypnut (ušetří ~1M PCF lookups/frame).
  // v74.77-revert: castShadow zpět ON — pixely pořád vrhají stín (na shadowGround/frame).
  ...
  // v74.77: Toon material sjednocuje vzhled bloků se zbytkem 3D scény (carriers/balls)
  ...
  // v74.77: gun (gunBody + gunHead) z GLB — async load
  _loadGun();

  state.ready = true;
  state._dirty = true; // v74.77: po init první render musí proběhnout
  // v74.77: PRE-WARM SHADER COMPILE — bez tohohle iOS Safari kompiluje shadery
...
// v74.77: gun — gunBody (statický korpus, posunuje se s cannonX) + gunHead
...
function triggerPixelDestroy(gridX, gridY, hexColor) {
  ...
  // v74.77: particles zapnuty na všech tierech — full shard count, flash všude.
...
function triggerPixelCA(gx, gy, hexColor) {
  ...
  // v74.77: CA zůstává i na LOW — cost je mizivý (1 extra draw call, fade 180ms)
  state._dirty = true;
...
function triggerDustBurst(gx, gy, hexColor) {
  ...
  // v74.77: dust zůstává i na LOW — cost je mizivý (1 extra draw call)
  state._dirty = true;
  const wx = gx * SCALE + SCALE / 2;
...
function triggerPixelHit(gx, gy) {
  ...
  // v74.77: LOW tier — hit bounce OFF. Stejný updateGrid full-rewrite mechanismus
...
function triggerPixelWave(gx, gy) {
  if (!state.ready) return;
  // v74.77: LOW tier — wave OFF. updateGrid re-write všech ~750 pixel positions
...
function updateAnimations(dt) {
  if (!state.ready || !state.shardMesh) return;
  // v74.77: pokud cokoli aktivního (shards/ghosts/dust/waves), označit scénu jako dirty
  ...
  for (let i = state.shards.length - 1; i >= 0; i--) {
    ...
    s.t += dt;
    if (s.t >= s.life) {
      // v74.77: swap-pop + release do poolu (žádný splice shift, žádný GC)
      _releaseShard(s);
  ...
  if (state.pixelBounce.size > 0) {
    ...
    if (anyActive && state._lastGrid && state._lastColors) {
      ...
      // v74.77: aktivní vlny posouvají Z pixelů → shadow map musí refreshnout
      if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
    }
  }
  // v74.77: aktivní shards (destrukce, sparks) také posouvají objekty → refresh shadow
...
function updateBlocks(blocks, COLORS) {
  ...
  state._dirty = true;
  // v74.77: blocks se mohou změnit (HP klesá, blok zničen) → shadow refresh
  if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
  const H = state.GH * SCALE;
...
function updateBlockOutlines(blocks) {
  ...
  // v74.77: skip rebuild pokud bloky se nezměnily. drawBlocks volá tohle při každé
...
// v74.77: dirty-flag render skipping. Top scéna se renderuje JEN když se něco
...
// v74.77: zjištění zda běží jakákoli 3D anim — beltLoop tím zařídí, aby render
...
function setVisible(visible) {
  ...
  if (visible) state._dirty = true; // v74.77: po zviditelnění vynucený refresh
}
...
if (typeof window !== 'undefined') {
  window.render3d = {
    ...
    updateBlockOutlines,
    updateProjectiles,
    setCannonPosition,    // v74.77
    triggerMuzzleFlash,   // v74.77
    triggerPixelDestroy,
    ...
    setQualityTier: (tier) => {
      ...
      // v74.77: top canvas zpět na plné 2× retina — B-pool refactor (v74.77)
      ...
      // v74.77: HIGH = plné stíny (512 PCFSoft), ALE on-demand (autoUpdate=false).
      ...
      // v74.77: recompile JEN když se shadow stav skutečně mění. Předtím `prev !== t`
      ...
      state._dirty = true; // v74.77: tier change → vynucený refresh
