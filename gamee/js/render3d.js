...
// v73.290: version stamp pro watchdog — game.js compare proti tomuto
if (typeof window !== 'undefined') window.BB_VERSION_R3D = 'v73.290';
...
function init(canvas, opts) {
  ...
  state.ready = true;
  state._dirty = true; // v73.290: po init první render musí proběhnout
  // v73.290: PRE-WARM SHADER COMPILE — bez tohohle iOS Safari kompiluje shadery
...
function updateAnimations(dt) {
  if (!state.ready || !state.shardMesh) return;
  // v73.290: pokud cokoli aktivního (shards/ghosts/dust/waves), označit scénu jako dirty
  ...
  if (state.pixelBounce.size > 0) {
    ...
    if (anyActive && state._lastGrid && state._lastColors) {
      ...
      // v73.290: aktivní vlny posouvají Z pixelů → shadow map musí refreshnout
      if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
    }
  }
  // v73.290: aktivní shards (destrukce, sparks) také posouvají objekty → refresh shadow
...
function updateBlocks(blocks, COLORS) {
  ...
  state._dirty = true;
  // v73.290: blocks se mohou změnit (HP klesá, blok zničen) → shadow refresh
  if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
  const H = state.GH * SCALE;
...
// v73.290: dirty-flag render skipping. Top scéna se renderuje JEN když se něco
...
function setVisible(visible) {
  ...
  if (visible) state._dirty = true; // v73.290: po zviditelnění vynucený refresh
}
...
if (typeof window !== 'undefined') {
  window.render3d = {
    ...
    setQualityTier: (tier) => {
      ...
      // v73.290: HIGH = plné stíny (512 PCFSoft), ALE on-demand (autoUpdate=false).
      ...
      state._dirty = true; // v73.290: tier change → vynucený refresh
