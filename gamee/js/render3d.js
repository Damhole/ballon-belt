...
// v73.316: version stamp pro watchdog — game.js compare proti tomuto
if (typeof window !== 'undefined') window.BB_VERSION_R3D = 'v73.316';
...
function init(canvas, opts) {
  ...
  state.ready = true;
  state._dirty = true; // v73.316: po init první render musí proběhnout
  // v73.316: PRE-WARM SHADER COMPILE — bez tohohle iOS Safari kompiluje shadery
...
function triggerPixelDestroy(gridX, gridY, hexColor) {
  ...
  // v73.316: particles zapnuty na všech tierech — full shard count, flash všude.
...
function triggerPixelCA(gx, gy, hexColor) {
  ...
  // v73.316: CA zůstává i na LOW — cost je mizivý (1 extra draw call, fade 180ms)
  state._dirty = true;
...
function triggerDustBurst(gx, gy, hexColor) {
  ...
  // v73.316: dust zůstává i na LOW — cost je mizivý (1 extra draw call)
  state._dirty = true;
  const wx = gx * SCALE + SCALE / 2;
...
function triggerPixelHit(gx, gy) {
  ...
  // v73.316: LOW tier — hit bounce OFF. Stejný updateGrid full-rewrite mechanismus
...
function triggerPixelWave(gx, gy) {
  if (!state.ready) return;
  // v73.316: LOW tier — wave OFF. updateGrid re-write všech ~750 pixel positions
...
function updateAnimations(dt) {
  if (!state.ready || !state.shardMesh) return;
  // v73.316: pokud cokoli aktivního (shards/ghosts/dust/waves), označit scénu jako dirty
  ...
  if (state.pixelBounce.size > 0) {
    ...
    if (anyActive && state._lastGrid && state._lastColors) {
      ...
      // v73.316: aktivní vlny posouvají Z pixelů → shadow map musí refreshnout
      if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
    }
  }
  // v73.316: aktivní shards (destrukce, sparks) také posouvají objekty → refresh shadow
...
function updateBlocks(blocks, COLORS) {
  ...
  state._dirty = true;
  // v73.316: blocks se mohou změnit (HP klesá, blok zničen) → shadow refresh
  if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
  const H = state.GH * SCALE;
...
// v73.316: dirty-flag render skipping. Top scéna se renderuje JEN když se něco
...
// v73.316: zjištění zda běží jakákoli 3D anim — beltLoop tím zařídí, aby render
...
function setVisible(visible) {
  ...
  if (visible) state._dirty = true; // v73.316: po zviditelnění vynucený refresh
}
...
if (typeof window !== 'undefined') {
  window.render3d = {
    ...
    setQualityTier: (tier) => {
      ...
      // v73.316: HIGH = plné stíny (512 PCFSoft), ALE on-demand (autoUpdate=false).
      ...
      state._dirty = true; // v73.316: tier change → vynucený refresh
