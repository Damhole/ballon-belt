...
// v74.59: version stamp pro watchdog — game.js compare proti tomuto
if (typeof window !== 'undefined') window.BB_VERSION_R3D = 'v74.59';
...
function init(canvas, opts) {
  ...
  // v74.59: Toon material sjednocuje vzhled bloků se zbytkem 3D scény (carriers/balls)
  ...
  // v74.59: gun (gunBody + gunHead) z GLB — async load
  _loadGun();

  state.ready = true;
  state._dirty = true; // v74.59: po init první render musí proběhnout
  // v74.59: PRE-WARM SHADER COMPILE — bez tohohle iOS Safari kompiluje shadery
...
// v74.59: gun — gunBody (statický korpus, posunuje se s cannonX) + gunHead
...
function triggerPixelDestroy(gridX, gridY, hexColor) {
  ...
  // v74.59: particles zapnuty na všech tierech — full shard count, flash všude.
...
function triggerPixelCA(gx, gy, hexColor) {
  ...
  // v74.59: CA zůstává i na LOW — cost je mizivý (1 extra draw call, fade 180ms)
  state._dirty = true;
...
function triggerDustBurst(gx, gy, hexColor) {
  ...
  // v74.59: dust zůstává i na LOW — cost je mizivý (1 extra draw call)
  state._dirty = true;
  const wx = gx * SCALE + SCALE / 2;
...
function triggerPixelHit(gx, gy) {
  ...
  // v74.59: LOW tier — hit bounce OFF. Stejný updateGrid full-rewrite mechanismus
...
function triggerPixelWave(gx, gy) {
  if (!state.ready) return;
  // v74.59: LOW tier — wave OFF. updateGrid re-write všech ~750 pixel positions
...
function updateAnimations(dt) {
  if (!state.ready || !state.shardMesh) return;
  // v74.59: pokud cokoli aktivního (shards/ghosts/dust/waves), označit scénu jako dirty
  ...
  if (state.pixelBounce.size > 0) {
    ...
    if (anyActive && state._lastGrid && state._lastColors) {
      ...
      // v74.59: aktivní vlny posouvají Z pixelů → shadow map musí refreshnout
      if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
    }
  }
  // v74.59: aktivní shards (destrukce, sparks) také posouvají objekty → refresh shadow
...
function updateBlocks(blocks, COLORS) {
  ...
  state._dirty = true;
  // v74.59: blocks se mohou změnit (HP klesá, blok zničen) → shadow refresh
  if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
  const H = state.GH * SCALE;
...
// v74.59: dirty-flag render skipping. Top scéna se renderuje JEN když se něco
...
// v74.59: zjištění zda běží jakákoli 3D anim — beltLoop tím zařídí, aby render
...
function setVisible(visible) {
  ...
  if (visible) state._dirty = true; // v74.59: po zviditelnění vynucený refresh
}
...
if (typeof window !== 'undefined') {
  window.render3d = {
    ...
    updateBlockOutlines,
    updateProjectiles,
    setCannonPosition,    // v74.59
    triggerMuzzleFlash,   // v74.59
    triggerPixelDestroy,
    ...
    setQualityTier: (tier) => {
      ...
      // v74.59: HIGH = plné stíny (512 PCFSoft), ALE on-demand (autoUpdate=false).
      ...
      state._dirty = true; // v74.59: tier change → vynucený refresh
