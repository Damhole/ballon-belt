...
if (typeof window !== 'undefined') window.BB_VERSION_R3D = 'v75.05';
...
// v74.79: Object pools pro particles — eliminuje GC pressure.
...
function init(canvas, opts) {
  ...
  // v74.79: receiveShadow vypnut (ušetří ~1M PCF lookups/frame).
  // v74.79-revert: castShadow zpět ON — pixely pořád vrhají stín (na shadowGround/frame).
  ...
  // v74.79: Toon material sjednocuje vzhled bloků se zbytkem 3D scény (carriers/balls)
...
// v74.79: gun — gunBody (statický korpus, posunuje se s cannonX) + gunHead
...
function triggerPixelDestroy(gridX, gridY, hexColor) {
  ...
  // v74.79: particles zapnuty na všech tierech — full shard count, flash všude.
...
function triggerDustBurst(gx, gy, hexColor) {
  ...
  // v74.79: dust zůstává i na LOW — cost je mizivý (1 extra draw call)
  state._dirty = true;
  const wx = gx * SCALE + SCALE / 2;
...
function updateAnimations(dt) {
  if (!state.ready || !state.shardMesh) return;
  // v74.79: pokud cokoli aktivního (shards/ghosts/dust/waves), označit scénu jako dirty
  ...
  for (let i = state.shards.length - 1; i >= 0; i--) {
    ...
    s.t += dt;
    if (s.t >= s.life) {
      // v74.79: swap-pop + release do poolu (žádný splice shift, žádný GC)
      _releaseShard(s);
  ...
  if (state.pixelBounce.size > 0) {
    ...
    if (anyActive && state._lastGrid && state._lastColors) {
      ...
      // v74.79: aktivní vlny posouvají Z pixelů → shadow map musí refreshnout
      if (state.sun && !state.sun.shadow.autoUpdate) state.sun.shadow.needsUpdate = true;
    }
  }
  // v74.79: aktivní shards (destrukce, sparks) také posouvají objekty → refresh shadow
...
function updateBlockOutlines(blocks) {
  ...
  // v74.79: skip rebuild pokud bloky se nezměnily. drawBlocks volá tohle při každé
...
// v74.79: dirty-flag render skipping. Top scéna se renderuje JEN když se něco
...
// v74.79: zjištění zda běží jakákoli 3D anim — beltLoop tím zařídí, aby render
...
if (typeof window !== 'undefined') {
  window.render3d = {
    ...
    updateBlockOutlines,
    updateProjectiles,
    setCannonPosition,    // v74.79
    triggerMuzzleFlash,   // v74.79
    triggerPixelDestroy,
    ...
    setQualityTier: (tier) => {
      ...
      // v74.79: top canvas zpět na plné 2× retina — B-pool refactor (v74.79)
      ...
      // v74.79: HIGH = plné stíny (512 PCFSoft), ALE on-demand (autoUpdate=false).
      ...
      // v74.79: recompile JEN když se shadow stav skutečně mění. Předtím `prev !== t`
