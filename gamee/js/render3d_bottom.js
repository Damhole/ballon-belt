...
// v73.343: version stamp pro watchdog
if (typeof window !== 'undefined') window.BB_VERSION_R3DB = 'v73.343';
...
// v73.343: cycle zkrácen — ball max uvnitř right box, nepřeteče (match game.js)
...
function init() {
  ...
  _loadBeltBoxes(contentGroup, toonGrad); // v73.343: 3D krabice po stranách belt
  _initUnifiedFrame();  // v73.54: belt + skulina + arena jako jeden 3D povrch

  st.ready = true;
  st._dirty = true; // v73.343: first frame render
  // v73.343: shader pre-warm — kompiluj všechny WebGL programy předem aby
...
// v73.343: GLB loader pro belt-side krabice (BeltBoxLeft, BeltBoxRight).
...
function _rebuildUnifiedFrame() {
  ...
  st.lastFrameKey = key;
  st._slotCacheValid = false; // v73.343: layout reálně mění slot pozice
...
function _initUnifiedFrame() {
  ...
  // v73.343: gradient injection — bottom (low Y, = nejnižší row) je světlejší ×1.18,
...
function updateCarriers(columns, colorsArr) {
  ...
  st._dirty = true; // v73.343: carriers update = layout change → vždy dirty

  // v73.103: rebuild frame pokud carriers pozice změnila (responzivní layout)
  _rebuildUnifiedFrame();

  // v73.343: per-slot measurement cache — během anim se layout nemění, takže
  ...
  for (let c = 0; c < colDivs.length && c < columns.length; c++) {
    ...
    for (let r = 0; r < slotDivs.length && r < col.length; r++) {
      ...
      if (wasHidden && !isHiddenVisual) {
        ...
        if (cboxR) {
          // v73.343: reveal trigger — 1× per slot reveal, neoptimalizujeme cache
      ...
      // v73.343: cache slot pos + dynScale (jinak getBoundingClientRect per slot per frame)
  ...
  // v73.343: track přítomnost mystery slotů — jejich textura scrolluje a tedy
...
// v73.343: dirty helper pro bottom scénu — krátký a explicit
function _bottomMarkDirty() { st._dirty = true; }
// v73.343: invalidace per-slot měření cache — volat při resize, level start, theme change
...
function updatePending(pendingArr, colorsArr) {
  if (!st.ready) return;
  // v73.343: dirty pokud má aspoň jednu pending ball (pohybují se každý frame)
...
function updateBelt(beltArr, beltAnim, colorsArr) {
  if (!st.ready) return;
  // v73.343: dirty pokud má aspoň jednu belt ball (belt anim posouvá)
  ...
  for (let i = 0; i < BELT_CAP; i++) {
    ...
    const xCSSrel = BELT_STARTX + (i * BELT_SPACING + offset) % BELT_TOTAL;
    // v73.343: visibility cull odstraněn. Balls jsou vždy v range 20..369,
...
// v73.343: dirty-flag render skipping pro bottom scénu — analogie top scene.
...
function updateWalls(columns) {
  ...
  st._dirty = true; // v73.343: walls update = layout/color change
...
// MIMO canvas (clipped).
function resize() {
  st._dirty = true; // v73.343: viewport change → refresh
  st._slotCacheValid = false; // v73.343: layout může změnit slot pozice
...
function refreshBeltTint() {
  ...
  // v73.343: inner barva odvozená z plate odd-tinted barvy + lighter (HSL L+0.28)
...
function setQualityTier(tier){
  ...
  st._dirty = true; // v73.343: tier change → vynucený refresh
