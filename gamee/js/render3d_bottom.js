...
// v73.290: version stamp pro watchdog
if (typeof window !== 'undefined') window.BB_VERSION_R3DB = 'v73.290';
...
function init() {
  ...
  st.ready = true;
  st._dirty = true; // v73.290: first frame render
  // v73.290: shader pre-warm — kompiluj všechny WebGL programy předem aby
...
function _rebuildUnifiedFrame() {
  ...
  st.lastFrameKey = key;
  st._slotCacheValid = false; // v73.290: layout reálně mění slot pozice
...
function updateCarriers(columns, colorsArr) {
  ...
  st._dirty = true; // v73.290: carriers update = layout change → vždy dirty

  // v73.103: rebuild frame pokud carriers pozice změnila (responzivní layout)
  _rebuildUnifiedFrame();

  // v73.290: per-slot measurement cache — během anim se layout nemění, takže
  ...
  for (let c = 0; c < colDivs.length && c < columns.length; c++) {
    ...
    for (let r = 0; r < slotDivs.length && r < col.length; r++) {
      ...
      if (wasHidden && !isHiddenVisual) {
        ...
        if (cboxR) {
          // v73.290: reveal trigger — 1× per slot reveal, neoptimalizujeme cache
      ...
      // v73.290: cache slot pos + dynScale (jinak getBoundingClientRect per slot per frame)
  ...
  // v73.290: track přítomnost mystery slotů — jejich textura scrolluje a tedy
...
// v73.290: dirty helper pro bottom scénu — krátký a explicit
function _bottomMarkDirty() { st._dirty = true; }
// v73.290: invalidace per-slot měření cache — volat při resize, level start, theme change
...
function updatePending(pendingArr, colorsArr) {
  if (!st.ready) return;
  // v73.290: dirty pokud má aspoň jednu pending ball (pohybují se každý frame)
...
function updateBelt(beltArr, beltAnim, colorsArr) {
  if (!st.ready) return;
  // v73.290: dirty pokud má aspoň jednu belt ball (belt anim posouvá)
...
// v73.290: dirty-flag render skipping pro bottom scénu — analogie top scene.
...
function updateWalls(columns) {
  ...
  st._dirty = true; // v73.290: walls update = layout/color change
...
// MIMO canvas (clipped).
function resize() {
  st._dirty = true; // v73.290: viewport change → refresh
  st._slotCacheValid = false; // v73.290: layout může změnit slot pozice
...
function setQualityTier(tier){
  ...
  st._dirty = true; // v73.290: tier change → vynucený refresh
