...
// v74.37: version stamp pro watchdog
if (typeof window !== 'undefined') window.BB_VERSION_R3DB = 'v74.37';
...
// v74.37: cycle zkrácen — ball max uvnitř right box, nepřeteče (match game.js)
...
function init() {
  ...
  _loadBeltBoxes(contentGroup, toonGrad); // v74.37: 3D krabice po stranách belt
  _initUnifiedFrame();  // v73.54: belt + skulina + arena jako jeden 3D povrch

  st.ready = true;
  st._dirty = true; // v74.37: first frame render
  // v74.37: shader pre-warm — kompiluj všechny WebGL programy předem aby
...
// v74.37: GLB loader pro belt-side krabice (BeltBoxLeft, BeltBoxRight).
...
function _rebuildUnifiedFrame() {
  ...
  st.lastFrameKey = key;
  st._slotCacheValid = false; // v74.37: layout reálně mění slot pozice
...
function _initUnifiedFrame() {
  ...
  // v74.37: gradient injection — bottom (low Y, = nejnižší row) je světlejší ×1.18,
...
function updateCarriers(columns, colorsArr) {
  ...
  st._dirty = true; // v74.37: carriers update = layout change → vždy dirty

  // v73.103: rebuild frame pokud carriers pozice změnila (responzivní layout)
  _rebuildUnifiedFrame();

  // v74.37: per-slot measurement cache — během anim se layout nemění, takže
  ...
  for (let c = 0; c < colDivs.length && c < columns.length; c++) {
    ...
    for (let r = 0; r < slotDivs.length && r < col.length; r++) {
      ...
      if (wasHidden && !isHiddenVisual) {
        ...
        if (cboxR) {
          // v74.37: reveal trigger — 1× per slot reveal, neoptimalizujeme cache
      ...
      // v74.37: cache slot pos + dynScale (jinak getBoundingClientRect per slot per frame)
  ...
  // v74.37: track přítomnost mystery slotů — jejich textura scrolluje a tedy
...
// v74.37: dirty helper pro bottom scénu — krátký a explicit
function _bottomMarkDirty() { st._dirty = true; }
// v74.37: invalidace per-slot měření cache — volat při resize, level start, theme change
...
function updatePending(pendingArr, colorsArr) {
  if (!st.ready) return;
  // v74.37: dirty pokud má aspoň jednu pending ball (pohybují se každý frame)
...
function updateBelt(beltArr, beltAnim, colorsArr) {
  if (!st.ready) return;
  // v74.37: dirty pokud má aspoň jednu belt ball (belt anim posouvá)
  ...
  for (let i = 0; i < BELT_CAP; i++) {
    ...
    const xCSSrel = BELT_STARTX + (i * BELT_SPACING + offset) % BELT_TOTAL;
    // v74.37: visibility cull odstraněn. Balls jsou vždy v range 20..369,
...
// v74.37: dirty-flag render skipping pro bottom scénu — analogie top scene.
...
function updateWalls(columns) {
  ...
  st._dirty = true; // v74.37: walls update = layout/color change
...
// MIMO canvas (clipped).
function resize() {
  st._dirty = true; // v74.37: viewport change → refresh
  st._slotCacheValid = false; // v74.37: layout může změnit slot pozice
...
function refreshBeltTint() {
  ...
  // v74.37: inner barva odvozená z plate odd-tinted barvy + lighter (HSL L+0.28)
...
function setQualityTier(tier){
  ...
  st._dirty = true; // v74.37: tier change → vynucený refresh
